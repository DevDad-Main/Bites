...
export const restaurantController = {
	...
	fetchRestaurantReviews: async (req: Request<{ restaurantId: string }>, res: Response, next: NextFunction) => {
		try {
			const { restaurantId } = req.params;
			...
			if (!reviewIds) {
				return sendError(res, "Unsuccessful redis lookup. No Review IDs Found.", 400, ["Please try to submit a review before attempting to fetch any data.", "No Data Found."])
			}
			...
			if (reviews.length === 0) {
				return sendSuccess(res, {}, "No Reviews Found, Please ensure you have submitted a review befre trying to fetch them.", 200)
			}
			...
			logger.error(error.message || "Something went wrong while fetching restaurant reviews data.", { error })
			next(error)
		}
	}
};
